...
DESCRIPTION = (
    ...
    "``scFilterStats`` estimates the percentage of reads that would be filtered given "
    "a set of criteria and outputs it to a tab-separated file. The following metrics "
    "are estimated according to the --binSize and --distanceBetweenBins parameters:\n"
    "* Total sampled reads (excluding unmapped)\n"
    ...
    "* Wrong motif (due to --motifFilter)\n"
    "* Unwanted GC content (due to --GCcontentFilter)\n"
    "* Low aligned fraction (due to --minAlignedFraction)\n\n"
    "Note that alignments are sampled from bins of size --binSize spaced "
    "--distanceBetweenBins apart."
)
