...
def plot_region(
    ...
    if region_mask.sum() == 0:
        raise ValueError(f"No features found in region {region}")
    ...
    if summarize:
        X = summarize_matrix_for_heatmap(X, dpi=dpi, figsize=figsize)[0]
    ...
    if X_plot.min == 0 and X_plot.max == 0:
        logging.info("All values in track plot are zero. Setting color limits to [0, 1].")
        map_min, map_max = 0, 1
