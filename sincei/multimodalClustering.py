...
def multiModal_clustering(mode1_adata, mode2_adata, column_key='barcode_nla', nK=20):
    ...
    corpus_lsi, cell_topic = LSA_gensim(mtx, list(mode2_adata.obs.index),
                                        list(mode2_adata.var.index), nTopics = 20, smartCode='lfu')
    # get graph
    ...
    optimiser.optimise_partition_multiplex([part_rna, part_chic],
                                           layer_weights=[2, 1],
                                           n_iterations=-1)
...
def umap_aligned(pca_mode1, pca_mode2, nK=15, distance_metric='eucledian'):
    ...
    # UMAP
    UA = umap.AlignedUMAP(spread = 10, min_dist=0.01, n_neighbors=nK, metric=distance_metric, init='random', random_state=42)
